...
expr locals l (Ir.Var v) = ExpAtom . fst =.< var locals l v
expr locals l (Ir.Apply e1 e2) = do
  e1 <- expr locals l e1
  e2 <- expr locals l e2
  return $ ExpApply e1 e2
expr locals l@(loc,_) (Ir.Let v (Ir.Var e) rest) = do
  -- this is a little hacky, but is generated by pattern matches
  (a, st) <- var locals (loc,Just v) e
  rest <- expr (Map.insert v st locals) l rest
  return $ ExpLet v (ExpAtom a) rest
expr locals l@(loc,_) (Ir.Let v e rest) = do
  e <- expr locals (loc,Just v) e
  rest <- expr (Map.insert v False locals) l rest
...
var :: Locals -> (SrcLoc, Maybe Var) -> Var -> State (Prog, Globals) (Atom, Bool)
var locals _ v | Just s <- Map.lookup v locals = return $ (AtomLocal v, s)
var _ (loc,_) v = do
  (prog, globals) <- get
