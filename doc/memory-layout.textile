...
In order to mix notions of "compile time" and "runtime", code representation constructs such as Exp must be able to directly include pointers to runtime values.  Since Exp has a fixed static type and values can have any type, we intorduce a modicum of dynamic typing via the @Any@ type.

The memory layout of @Any@ is equivalent to @(TypeVal, t)@, with the invariant that the first element exactly describes the type of the second element.  As a consequence, @Any@ has variable size.
